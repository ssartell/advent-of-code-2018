...
var run = teams => {
    var immuneSystem = R.forEach(x => x.team = 'immuneSystem', teams[0]);
    var infection = R.forEach(x => x.team = 'infection', teams[1]);
    var allGroups = R.concat(immuneSystem, infection);
    
    while (R.any(hasUnits, infection) && R.any(hasUnits, immuneSystem)) {
        var targets = getTargetSelection(allGroups);
        attack(allGroups, targets);
    }
    
    return R.pipe(R.filter(hasUnits), R.map(x => x.units), R.sum)(allGroups);
}
